...
- **QA-HAPPY-02 — Dado persistido bate com o que a tela/resposta afirma.** Gatilho: o `EP` toca `MUT`. **Verifique de verdade em modo leitura, não suponha:** rode `SELECT`s (estado atual, contagens, joins) confirmando que o que foi gravado corresponde ao que a resposta declara. Conexão **read-only** (`SET SESSION TRANSACTION READ ONLY`, usuário somente-SELECT, ou `DB::select()` via Tinker). **Nunca INSERT/UPDATE/DELETE/DDL — nem em transação com rollback** (commits implícitos de DDL, triggers e auto-increment vazam mesmo com throw). **Proibido `php artisan test`.**
