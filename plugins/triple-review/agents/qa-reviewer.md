...
- **QA-BLOCK-03 — Duplo submit / duplo clique gera duplicidade ou reprocessamento indevido.** Gatilho: `EP` que **cria um registro OU muta um status/transição sensível** (finalizar, registrar, aprovar, descartar, cancelar — qualquer escrita que não deveria acontecer duas vezes) sem idempotência/lock. Dois cliques criam dois registros do mesmo fato, ou reprocessam a mesma transição gerando efeito colateral duplicado (segundo e-mail, segunda linha de histórico, segunda tentativa de mudar um status que já mudou)? `FAIL` = falta tanto o lock/reverificação sob transação quanto um mecanismo de bloqueio no submit (botão desabilitado, submit assíncrono que desabilita). (Exemplos de duplicidade proibida do projeto: seção "Regras críticas do domínio (QA)" do overlay.)
